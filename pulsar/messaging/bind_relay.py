...
def build_relay_transport(manager, relay_url, conf):
    ...
    Hoisted out of :func:`bind_manager_to_relay` so the capability
    publisher can share the same transport instance instead of building
    a second one with duplicate auth state.
    """
...
def bind_manager_to_relay(manager, relay_state: RelayState, relay_url, conf, relay_transport=None):
    ...
    Args:
        ...
        relay_transport: Optional pre-built ``RelayTransport``; if omitted,
            one is constructed via :func:`build_relay_transport`. Sharing
            a transport across binders avoids redundant token fetches.
    """
...
def publish_manager_capabilities_to_relay(app, manager, relay_transport, conf):
    """POST this manager's capability snapshot to its relay topic.

    A single POST per pulsar startup. The relay's topic streams have no
    per-message TTL, so the snapshot stays available for any Galaxy that
    queries the topic later via the REST messages endpoint. We do not
    re-publish on a heartbeat — Galaxy fetches synchronously per use,
    and the ``capabilities_by_manager`` payload is static for the
    process lifetime.

    Failures are logged but never raised: capabilities are advisory and
    must not prevent the manager's control consumers from coming up.
    """
    ...
    if capabilities is None:
        log.warning(
            "No cached capabilities for manager %s; skipping relay publish.",
            manager.name,
        )
        return
    relay_topic_prefix = conf.get('relay_topic_prefix', '')
...
def __make_capabilities_topic_name(prefix, manager_name):
    """Topic name for capability snapshots — mirrors ``__make_topic_name``.
